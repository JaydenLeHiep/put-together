...
export default function AdminCoursesPage() {
    ...
    const [selected, setSelected] = useState<LessonInCourse | null>(null);
    ...
    const [openStatus, setOpenStatus] = useState({ draft: true, published: true });
    ...
    const canEditLesson = useMemo(() => {
        ...
        return user.id === selected.createdById;
    }, [selected, user?.id]);
    ...
    async function handlePublish() {
        ...
        try {
            ...
            if (selected) {
                setSelected({ ...selected, isPublished: true });
            }
        } catch (error) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-3 text-sm text-gray-500">
                                            ...
                                            <span
                                                className={`px-2 py-1 rounded-full text-xs font-medium ${selected.isPublished
                                                        ? "bg-green-100 text-green-700"
                                                        : "bg-yellow-100 text-yellow-700"
                                                    }`}
                                            >
                                                {selected.isPublished ? "Veröffentlicht" : "Entwurf"}
                                            </span>
                                        </div>
                                    </div>
            ...
            <Modal
                ...
                message={`Sind Sie sicher, dass Sie die Lektion "${selected?.title}" löschen möchten? Diese Aktion kann nicht rückgängig gemacht werden.`}
                confirmText="Löschen"
        ...
        <div className="px-3 mb-2">
            ...
            >
                <span className="flex items-center gap-2">
                    <svg
                        className={`w-4 h-4 text-gray-600 transition-transform duration-200 ${isCategoryOpen ? "rotate-90" : ""
                            }`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
    ...
    return (
        <div className="mb-1">
            ...
            >
                <span className="flex items-center gap-2">
                    <svg
                        className={`w-4 h-4 text-lila-600 transition-transform duration-200 ${isOpen ? "rotate-90" : ""
                            }`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                ...
                <div className="mt-1 ml-3 space-y-0.5">
                    {lessons.map((l) => (
                        <button
                            key={l.id}
                            onClick={() => onSelect(l)}
                            className={`block w-full text-left px-4 py-2.5 text-sm rounded-lg transition-all duration-150 ${selected?.id === l.id
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <div
                                    className={`w-1.5 h-1.5 rounded-full ${selected?.id === l.id ? "bg-lila-600" : "bg-gray-300"
                                        }`}
                                />
